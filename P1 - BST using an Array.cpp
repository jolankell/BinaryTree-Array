////
// Name: Nolan Kelly
// Assignment:   P1 - Array based BST
// Description: This program reads in information on computer scientists
// from a file, inserts them into a binary search tree sorted by the
// scientists ID's, and removes scientists with ID's 101-110 from the tree.
// Prints are made to the screen displaying the actions being taken on the
// tree.
////
...
int main() {
	...
	for (int i = 0; i < list.size(); i++)
	{
		...
		tree2.insert(list[i], list[i]->getID(), rootIndex);
	}
	...
	tree.insert(new ComputerScientist("Robert", "Ward", "Education", 50), 50, rootIndex);
	tree.insert(new ComputerScientist("Grace", "Hopper", "Compilers", 40), 40, rootIndex);
	tree.insert(new ComputerScientist("Alan", "Turing", "Cryptogarphy", 60), 60, rootIndex);
	tree.insert(new ComputerScientist("John", "von Neumann", "Hardware", 45), 45, rootIndex);
	tree.insert(new ComputerScientist("Dennis", "Ritchie", "Compilers", 55), 55, rootIndex);
	tree.insert(new ComputerScientist("Ada", "Lovelace", "Programming", 30), 30, rootIndex);
	tree.insert(new ComputerScientist("Edsger", "Dikstra", "Algorithms", 80), 80, rootIndex);
	tree.insert(new ComputerScientist("Ken", "Thompson", "OS", 70), 70, rootIndex);
	tree.insert(new ComputerScientist("Lonnie", "Heinke", "Education", 58), 58, rootIndex);
